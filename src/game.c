...
void game_init(game_t *p_game) {
	...
	p_game->img.gold = image_new(&p_game->assets[ASSET_ICONS], 2, 2);
...
void game_render_placing_ui(game_t *p_game) {
	...
	block_renderer_render_plain_sprite(&p_game->block_renderer, p_game->img.outline.dest.x + 2,
	                                   p_game->img.outline.dest.y + 2,
...
void game_render_mode_ui(game_t *p_game) {
	game_render_text_center_x(p_game, SCREEN_H - p_game->font.ch_h * 2, game_mode_name(p_game));
}
...
void game_events_ingame(game_t *p_game) {
	...
	if (p_game->mode == MODE_PLACING) {
		...
		if (button_pressed(&p_game->button.left)) {
			...
			p_game->cursor_block.type = blocks_shop[p_game->shop_pos];
